...
def draw_bboxes_and_graph(source_image, G, offset, operators, counter=0):
    ...
    for node in G.nodes():
        if G.node[node]['is_matched'] is True:
            ...
            cv2.circle(source_image, tuple(G.node[node]['operators']['CENTRE_POINT']), 2, color, 3)
            cv2.putText(source_image, label, (x1, y1 + t_size[1] + 4), cv2.FONT_HERSHEY_PLAIN, 1, [255, 255, 255], 2)

    for edge in list(G.edges.data()):
        from_node = edge[0]
        to_node = edge[1]
        edge_operators = edge[2]['operators']
        from_node_operators = G.nodes[from_node]['operators']
        to_node_operators = G.nodes[to_node]['operators']

        for operator in operators['edge']:
            if operator != 'CENTRE_POINT':
                if (edge_operators[operator] == 1):
                    cv2.arrowedLine(source_image, tuple(from_node_operators['CENTRE_POINT']),
                                    tuple(to_node_operators['CENTRE_POINT']),
                                    (0, 255, 0), thickness=3, line_type=8)
                    cv2.putText(source_image, operator, tuple(edge_operators['CENTRE_POINT']), cv2.FONT_HERSHEY_PLAIN, 1, [255, 255, 255], 2)
                if (edge_operators[operator] == -1):
                    cv2.arrowedLine(source_image, tuple(to_node_operators['CENTRE_POINT']),
                                    tuple(from_node_operators['CENTRE_POINT']),
                                    (0, 255, 0), thickness=3, line_type=8)
                    cv2.putText(source_image, operator, tuple(edge_operators['CENTRE_POINT']), cv2.FONT_HERSHEY_PLAIN, 1, [255, 255, 255], 2)
...
def get_graph_image(G):
    ...
    nx.draw_networkx(G, node_size=1500, nodelist=matched_nodes, labels=node_labels, node_color='yellow',
